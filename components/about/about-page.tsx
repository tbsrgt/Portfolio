    ...
    <>
      <section className="mx-auto w-full max-w-300 px-6 pt-40 pb-20 sm:px-10 sm:pt-52 sm:pb-28">
        <FadeIn className="flex flex-wrap items-center gap-4">
          <Portrait alt={i18n.about.photoAlt} className="aspect-square w-14 rounded-full" />
        ...
        <FadeIn delay={0.2} className="mt-10 grid gap-8 lg:mt-14 lg:grid-cols-2">
          <div aria-hidden="true" className="bg-foreground/10 hidden h-px self-center lg:block" />
          <div>
            <p className="text-foreground/70 max-w-[46ch] text-lg leading-relaxed sm:text-xl">{copy.lead}</p>
      ...
      <section aria-labelledby="principles-heading" className="mx-auto w-full max-w-300 px-6 pb-24 sm:px-10 sm:pb-36">
        <Reveal>
      ...
      <section aria-labelledby="story-heading" className="bg-foreground text-background w-full py-24 sm:py-36">
          ...
          <ol className="flex flex-col">
            {copy.chapters.map((chapter, index) => (
              <li key={chapter.place}>
                <Reveal delay={0.04 * index} className="border-background/15 grid gap-4 border-t py-8 sm:grid-cols-[9rem_1fr] sm:gap-8 sm:py-10">
                  <p className="text-background/55 text-sm tabular-nums">{chapter.year}</p>
                  <div>
                    <p className="text-background/70 flex items-center gap-3 text-sm">
                      <ChapterLogo src={chapter.logo} fill={"logoFill" in chapter && chapter.logoFill} name={chapter.place} />
                      {chapter.place}
                    </p>
                    <h3 className="mt-4 text-2xl font-medium tracking-tight sm:text-[1.75rem]">{chapter.title}</h3>
                    <p className="text-background/65 mt-2 max-w-[52ch] leading-relaxed">{chapter.text}</p>
                  </div>
                </Reveal>
              </li>
            ))}
      ...
      <section aria-labelledby="tools-heading" className="mx-auto grid w-full max-w-300 items-center gap-10 px-6 py-24 sm:px-10 sm:py-36 lg:grid-cols-2 lg:gap-16">
        <Reveal>
          <h2 id="tools-heading" className={sectionHeading}>
            {copy.toolsHeading}
          </h2>
          <p className="text-foreground/65 mt-5 max-w-[44ch] text-lg leading-relaxed">{copy.toolsText}</p>
        </Reveal>
        <Reveal delay={0.1}>
          <Stack />
        </Reveal>
      ...
      <section aria-labelledby="work-heading" className="mx-auto w-full max-w-300 px-6 pb-12 sm:px-10">
        <Reveal className="flex flex-wrap items-end justify-between gap-4">
          <h2 id="work-heading" className={sectionHeading}>
            {copy.workHeading}
          </h2>
          <Link href="/projects" className="focus-ring group text-foreground inline-flex min-h-11 items-center gap-2 rounded-lg text-sm font-medium underline underline-offset-4">
            {copy.allWork}
            <ArrowRight className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-0.5" aria-hidden="true" />
          </Link>
        </Reveal>
          ...
          {work.map((project, index) => (
            <li key={project.slug}>
              <Reveal delay={index * 0.06}>
                <a href={project.url} target="_blank" rel="noopener noreferrer" className="focus-ring group block rounded-3xl">
                  ...
                  <div className="mt-4 flex items-start justify-between gap-4 px-1">
                    <div>
                      <p className="text-foreground text-lg font-medium tracking-tight">{project.title[locale]}</p>
                      <p className="text-foreground/55 text-sm">{project.sector[locale]}</p>
                    </div>
                    <ArrowUpRight className="text-foreground/50 group-hover:text-foreground mt-1 h-5 w-5 transition-[color,transform] duration-300 group-hover:rotate-45" aria-hidden="true" />
                  </div>
                </a>
              </Reveal>
            </li>
...
function ChapterLogo({ src, fill, name }: { src: string; fill: boolean; name: string }): ReactNode {
  if (!src) {
    return (
      <span aria-hidden="true" className="bg-background/10 inline-flex h-8 w-8 items-center justify-center rounded-lg text-xs font-semibold">
        {name.charAt(0)}
      </span>
    );
  }
  return (
    <span aria-hidden="true" className={`inline-flex h-8 w-8 overflow-hidden rounded-lg bg-white ${fill ? "" : "p-1"}`}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
      <img src={src} alt="" width={32} height={32} className={`h-full w-full ${fill ? "object-cover" : "object-contain"}`} />
    </span>
  );
}
