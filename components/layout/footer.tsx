...
const linkClass = "focus-ring text-foreground/65 hover:text-foreground rounded transition-colors";

export function Footer(): ReactNode {
  const { copy } = useLanguage();

  return (
    <footer className="mx-auto w-full max-w-275 px-6 pt-4 pb-32 sm:px-10">
      <div className="border-foreground/10 grid gap-10 border-t pt-10 sm:grid-cols-2 md:grid-cols-[1.4fr_1fr_1fr]">
        <div>
          <p className="text-foreground text-lg font-medium tracking-tight">{site.name}</p>
          <p className="text-foreground/55 mt-2 max-w-[36ch] text-sm leading-relaxed">{copy.footer.tagline}</p>
        ...
        <nav aria-label={copy.footer.navigation}>
          <p className="text-foreground text-sm font-semibold">{copy.footer.navigation}</p>
        ...
        <div>
          <p className="text-foreground text-sm font-semibold">{copy.footer.contact}</p>
          <ul className="mt-4 flex flex-col gap-2.5 text-sm">
      ...
      <div className="text-foreground/45 mt-10 flex flex-wrap items-center justify-between gap-3 text-xs">
        ...
        <div className="flex flex-wrap gap-4">
          <Link href="/mentions-legales" className="focus-ring hover:text-foreground rounded underline underline-offset-4">
            {copy.footer.legal}
          </Link>
