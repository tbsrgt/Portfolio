...
export function ProjectCard({ project, index, total }: Props): ReactNode {
  ...
  return (
    <article
      className="project-panel border-foreground/8 relative overflow-hidden rounded-[2rem] border"
      style={{ "--accent": project.accent } as CSSProperties}
    >
      <div className="grid items-center gap-6 p-3 sm:p-5 lg:grid-cols-[1.35fr_1fr] lg:gap-12 lg:p-8">
        <div className={flipped ? "lg:order-2" : undefined}>
          <Preview project={project} alt={alt} host={host} visitLabel={copy.visitProject} />
        </div>
        ...
        <div className="flex flex-col px-3 pb-4 sm:px-2 lg:py-4">
          ...
          <h3 className="text-foreground mt-5 font-serif text-[2.5rem] leading-[0.95] font-medium tracking-tight sm:text-[3.25rem]">
            {title}
          </h3>
          <p className="text-foreground/55 mt-2 text-sm">{project.place}</p>

          <p className="text-foreground/75 mt-6 text-[17px] leading-relaxed">{project.summary[locale]}</p>

          <ul className="border-foreground/10 mt-6 flex flex-col border-t">
            {project.highlights[locale].map((highlight) => (
              <li key={highlight} className="border-foreground/10 text-foreground/80 flex gap-3 border-b py-3 text-[15px]">
          ...
          {project.url ? (
            ...
            <p className="text-foreground/55 mt-8 text-sm">{copy.previewOnRequest}</p>
          )}
        </div>
      </div>
    </article>
...
function Preview({ project, alt, host, visitLabel }: { project: Project; alt: string; host: string | null; visitLabel: string }): ReactNode {
  ...
  const frame = (
    <>
      <div className="border-foreground/10 bg-background overflow-hidden rounded-xl border shadow-[0_30px_60px_-30px_rgb(0_0_0/0.4)] sm:rounded-2xl">
        <div className="border-foreground/8 flex h-7 items-center gap-1.5 border-b px-3 sm:h-8" aria-hidden="true">
      ...
      >
        <div className="relative aspect-[390/844]">
          <Image src={`/projects/${project.slug}-mobile.webp`} alt="" fill sizes="140px" className="object-cover object-top" />
        </div>
      </div>
    </>
  );

  const stage = "project-stage group/preview relative block rounded-[1.4rem] px-4 pt-4 pb-12 sm:px-8 sm:pt-8 sm:pb-16";
  ...
  return (
    ...
    >
      ...
      <motion.span
        ...
        style={{ x: reduceMotion ? x : springX, y: reduceMotion ? y : springY, translateX: "-50%", translateY: "-50%" }}
        initial={false}
